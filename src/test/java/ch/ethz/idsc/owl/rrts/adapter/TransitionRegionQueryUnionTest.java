...
public class TransitionRegionQueryUnionTest extends TestCase {
  public void testSimple() throws ClassNotFoundException, IOException {
    ...
    {
      Transition transition = RnTransitionSpace.INSTANCE.connect(Tensors.vector(-2, 0), Tensors.vector(-2, 1));
    ...
    {
      Transition transition = RnTransitionSpace.INSTANCE.connect(Tensors.vector(0, -2), Tensors.vector(0, 2));
      assertFalse(transitionRegionQuery.isDisjoint(transition));
      assertFalse(trq1.isDisjoint(transition));
      assertTrue(trq2.isDisjoint(transition));
    }
    {
      Transition transition = RnTransitionSpace.INSTANCE.connect(Tensors.vector(2, -2), Tensors.vector(2, 2));
      assertFalse(transitionRegionQuery.isDisjoint(transition));
      assertTrue(trq1.isDisjoint(transition));
