...
public class IntersectionEntryFinderTest extends TestCase {
  public void testSE2() {
    ...
    Optional<Tensor> waypointSE2 = finder.on(TrajectoryEntryFinderTest.WAYPOINTS).apply(VectorNorm2.of(goal2D)).point();
  ...
  public void test2D() {
    ...
    Optional<Tensor> waypoint2D = finder.on(waypoints).apply(VectorNorm2.of(goal2D)).point();
  ...
  public void testSweep1() {
    ...
    Tensor distances = Tensor.of(TrajectoryEntryFinderTest.WAYPOINTS.stream().map(Extract2D.FUNCTION).map(VectorNorm2::of));
