...
public class DubinsTransitionSpaceTest extends TestCase {
  public void testLengthUnitless() throws ClassNotFoundException, IOException {
    RrtsNode start = RrtsNode.createRoot(Tensors.fromString("{1, 2}").append(Pi.HALF), RealScalar.ZERO);
  ...
  public void testLengthUnits() {
    RrtsNode start = RrtsNode.createRoot(Tensors.fromString("{1[m], 2[m]}").append(Pi.HALF), RealScalar.ZERO);
  ...
  public void testSamples() {
    RrtsNode start = RrtsNode.createRoot(Tensors.fromString("{2, 1, 0}"), RealScalar.ZERO);
