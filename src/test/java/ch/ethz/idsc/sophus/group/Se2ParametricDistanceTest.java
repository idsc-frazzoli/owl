...
public class Se2ParametricDistanceTest extends TestCase {
  ...
  public void testSimpleUnits() {
    Scalar scalar = Se2ParametricDistance.of(Tensors.fromString("{1[m], 2[m], 3}"), Tensors.fromString("{4[m], 6[m], 3}"));
    assertEquals(scalar, Quantity.of(5, "m"));
  }

  public void testOtherUnits() {
    Scalar scalar = Se2ParametricDistance.of(Tensors.fromString("{1[m], 2[m], 3}"), Tensors.fromString("{4[m], 6[m], 3.3}"));
