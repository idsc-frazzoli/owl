...
public class BiinvariantMeanIIRnNEW implements TensorUnaryOperator {
  ...
  public static TensorUnaryOperator of(GeodesicDisplay geodesicDisplay, ScalarUnaryOperator smoothingKernel, int radius, Scalar alpha) {
    return new BiinvariantMeanIIRnNEW(//
  ...
  /* package */ BiinvariantMeanIIRnNEW(GeodesicDisplay geodesicDisplay, ScalarUnaryOperator smoothingKernel, int radius, Scalar alpha) {
  ...
  public Tensor apply(Tensor x) {
    Tensor value = boundedLinkedList.size() < 2 //
        ? x.copy()
        : geodesicInterface.split(biinvariantMean.mean(Tensor.of(boundedLinkedList.stream()),
