...
/* package */ class R2KrigingDemo extends A1KrigingDemo {
  ...
  public R2KrigingDemo() {
    {
      spinnerRes.setArray(10, 20, 30, 50);
  ...
  public void render(GeometricLayer geometricLayer, Graphics2D graphics) {
    ...
    IntStream.range(0, rows).parallel().forEach(cx -> {
      for (int cy = 0; cy < cols; ++cy)
        array[cy][cx] = (Scalar) kriging.estimate(Tensors.of(dx.get(cx), dy.get(cy)));
    });
    Tensor arrayplot = ArrayPlot.of(Tensors.matrix(array), ColorDataGradients.CLASSIC);
    ImageRender imageRender = ImageRender.range(ImageFormat.of(arrayplot), Tensors.vector(rad, rad));
