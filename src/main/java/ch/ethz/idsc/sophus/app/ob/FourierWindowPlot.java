...
/* package */ class FourierWindowPlot {
  ...
  private static void plot(Tensor data, int radius, String signal, SmoothingKernel smoothingKernel) throws IOException {
    ...
    VisualRow visualRow = visualSet.add(//
        xAxis, //
        decibelConversion(Tensor.of(linearResponse(smoothingKernel, radius).append(linearResponse(smoothingKernel, radius)).flatten(1))
  ...
  private static void process(List<String> listData, Map<LieGroupFilters, TensorUnaryOperator> map, int radius, int limit, SmoothingKernel smoothingKernel)
    ...
    plot(Mean.of(smoothedX), radius, "x", smoothingKernel);
    plot(Mean.of(smoothedY), radius, "y", smoothingKernel);
    plot(Mean.of(smoothedA), radius, "a", smoothingKernel);
  }
